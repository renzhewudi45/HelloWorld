...
public class PhotoHelper {
	...
	public AlbumGetResponseBean getAlbums(AlbumGetRequestParam albumsRequest)
		 ...
		 * 获取照片信息
		 */
		List<AlbumBean> abs = albumsResponse.getAlbums();
		for (int i = 0; i < abs.size();i++) {
			AlbumBean abean = abs.get(i);
			try {
				result = renren.requestJSON(createGetPhotosParams(abean.getAid()+""));
				Log.i("test photos", result);
			} catch (RuntimeException e) {
				Util.logger("exception in getting photos:error in internet requesting\t"
						+ e.getMessage());
				throw new Throwable(e);
			}
			// 检查请求返回字符串是否错误信息
			Util.checkResponse(result, Renren.RESPONSE_FORMAT_JSON);
			ParsePhotoResult(result,abean.getPhotos());
		}
		// 成功获取相册，记录日志 
	...
	private Bundle createGetPhotosParams(String aid) {
		Bundle params = new Bundle();
		params.putString("method", "photos.get");
		params.putString("format", Renren.RESPONSE_FORMAT_JSON);
		params.putString("uid", String.valueOf(renren.getCurrentUid()));
		params.putString("aid",aid);
		params.putString("page","1");
		params.putString("count","199");
		return params;
	}
	
	private void ParsePhotoResult(String result,List<PhotoBean> photos) {
		if (result == null) {
			return;
		}
			...
			try { 
				...
				for (int i = 0; i < size; i++) {
					...
					if (object != null) {
						...
						photo.setUrlMain(object.optString("url_large"));
						photo.setCaption(object.optString("caption"));
						photo.setFileName(object.optString("url_large"));
						photo.setCreateTime(sdf.parse(object.optString("time")));
						photos.add(photo);
					}
				}
